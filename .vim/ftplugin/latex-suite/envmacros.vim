...
let s:frame = "\\begin{frame}\<cr>\\frametitle{<+title+>}\<cr><++>\<cr>\\end{frame}"
let s:block = "\\begin{block}{<+title+>}\<cr><++>\<cr>\\end{block}"
...
" }}}
" Beamer {{{
call s:Tex_SpecialMacros('BFR', '', 'frame', s:frame)
call s:Tex_SpecialMacros('BBL', '', 'block', s:block)
"}}}
